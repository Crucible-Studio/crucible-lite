...
`crucible/hybrid/` module roles (Amendment 13 — Hybrid Execution Tiers):
- `router.py` — RAG tier filter; reads `contract.retrieves` from agent frontmatter to enforce corpus access boundaries
- `ollama_client.py` — local LLM wrapper (Ollama); use for tasks that must not send PRIVATE content to a cloud API
- Tier index: `docs/hybrid/corpus_index.json`; classification guide: `docs/hybrid/corpus_classification.md`
